...
def edge_to_sense(edge):
    return {"r": "rise", "f": "fall"}.get(edge, "")


def expected_delta_mode(analysis_type):
...
def validate_delta_modes(blocks, analysis_type):
    ...
    for context_id, block in blocks.items():
        ...
        for sense in ("rise", "fall"):
            delay = block.get("{0}_delta_delay".format(sense), "")
            mode = block.get("{0}_delta_mode".format(sense), "")
            if not delay:
                continue
            if not mode:
                missing.append(str(context_id))
            elif mode != expected:
                modes.append("{0}:{1}".format(sense, mode))
        if modes:
            mismatches.append("context_id={0} modes={1}".format(context_id, ",".join(modes)))

    if missing or mismatches:
        print(
            "ERROR: report_delay_calculation delta mode mismatch for analysis_type={0}; "
            "expected Annotated {1} net delta delay.".format(analysis_type, expected),
            file=sys.stderr,
        )
        for item in mismatches[:20]:
            print("  mismatch {0}".format(item), file=sys.stderr)
        if len(mismatches) > 20:
            print("  ... {0} more mismatches".format(len(mismatches) - 20), file=sys.stderr)
        if missing:
            print("  missing mode contexts: {0}".format(", ".join(missing[:20])), file=sys.stderr)
            if len(missing) > 20:
                print("  ... {0} more missing-mode contexts".format(len(missing) - 20), file=sys.stderr)
        raise SystemExit(1)
...
def main():
    ...
    path_rows_file = sys.argv[1]
    context_file = sys.argv[2]
    raw_report = sys.argv[3]
    summary_out = sys.argv[4]
    active_out = sys.argv[5]
    ...
    with open(path_rows_file, "rb") as pf, open(active_out, "wb") as of:
    ...
    print("path_victim_rows_in={0}".format(rows_in))
    print("rows_out={0}".format(rows_out))
    print("active_aggressor_rows={0}".format(active_rows))
    print("victim_only_no_active_rows={0}".format(victim_only_rows))
    print("missing_context_rows={0}".format(missing_context))
    print("status_counts={0}".format(dict(status_counts)))
    print("summary={0}".format(summary_out))
    print("active_features={0}".format(active_out))
    return 0
