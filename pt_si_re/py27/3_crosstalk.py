...
CODE_INFO = {
    ...
    "W-NOXT":     ("crosstalk 열이 비어 있습니다 (net_attr.txt 를 못 읽음)",
                   "pt_shell 에서 dump_attr.tcl 을 돌려 net_attr.txt 를 만들어 주세요."),
    "W-NOPINATTR": ("arrival/slew 열이 비어 있습니다 (pin_attr.txt 를 못 읽음)",
                   "pt_shell 에서 dump_attr.tcl 을 돌려 pin_attr.txt 를 만들어 주세요."),
    "W-PARTIAL":  ("속성이 일부 줄에만 붙었습니다",
                   "리포트를 다시 뽑은 뒤 dump_attr.tcl 을 다시 돌리지 않으면 "
                   "이렇게 됩니다. 지금 리포트로 다시 만들어 주세요."),
...
def main():
    ...
    # slack 은 경로 블록의 **맨 끝** 줄에 나온다. 행을 만나는 즉시 쓰면 그 경로의
    # 마지막 한 줄에만 slack 이 들어가고 나머지는 빈다(실제로 2%만 채워졌다).
    # 그래서 경로 단위로 모아 두었다가, 경로가 끝나면 그때 한꺼번에 쓴다.
    # meta 는 경로마다 하나씩 만들어지고 slack 을 나중에 채우므로, 모아 둔 행들이
    # 같은 meta 를 보고 있어 flush 시점에는 값이 들어와 있다.
    with open(out, "w") as fh:
        ...
        def flush():
            for (p_idx, meta, a_idx, pin, cell, edge, net) in buf:
                na = net_tab.get(net, {})
                pa = pin_tab.get(pin, {})
                row = ([args.corner] if args.corner else []) + [
                    p_idx, meta["startpoint"], meta["endpoint"],
                    meta["path_group"], meta["path_type"],
                    meta["slack_status"], meta["slack"],
                    a_idx, pin, cell, edge, net,
        ...
        for path_idx, meta, arc_idx, pin, cell, edge, net in iter_arcs(rpt):
            ...
            try:
                ...
                pass
            buf.append((path_idx, meta, arc_idx, pin, cell, edge, net))
            n_rows += 1
        flush()
    ...
    if not net_tab:
        code("W-NOXT",
             "[ 주의 ] crosstalk 열이 하나도 없습니다.",
             "         net_attr.txt 가 없거나 읽지 못했습니다.")
    if not pin_tab:
        code("W-NOPINATTR",
             "[ 주의 ] arrival/slew 열이 하나도 없습니다.",
             "         pin_attr.txt 가 없거나 읽지 못했습니다.")
    # 일부만 붙는 경우가 가장 위험하다. 리포트를 새로 뽑고 dump_attr.tcl 을 다시
    # 안 돌리면 옛 속성 파일에 없는 넷이 조용히 빈칸으로 남는다. 전부 실패하면
    # E-NETNAME 으로 잡히지만, 일부만 실패하면 예전에는 아무 표시가 없었다.
    if net_tab and n_net_miss > 0:
        pct = 100.0 * n_net_miss / max(n_net_hit + n_net_miss, 1)
        if pct > 1.0:
            code("W-PARTIAL",
                 "[ 주의 ] 넷 속성이 %s줄에서 안 붙었습니다 (%.0f%%)."
                 % (format(n_net_miss, ","), pct),
                 "         net_attr.txt 에 그 넷들이 없습니다.")
