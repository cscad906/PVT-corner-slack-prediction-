# PrimeTime 목표 코너 자동 제외 scaling
# 새 pt_shell에서 아래 한 줄로 실행합니다.
#   source /home/KNUEEhdd1/sogang1/hyunss/PVT/pt_scaling/auto_scaling.tcl
#
# 아래 USER SETTINGS의 따옴표 안 값과 숫자만 바꾸면 됩니다.

# ======================= USER SETTINGS ========================

# 먼저 "check"로 선택 내용을 확인하고, 이상 없으면 "run"으로 바꾸세요.
set ACTION "check"
...
set RESULT_FOLDER "/home/KNUEEhdd1/sogang1/hyunss/PVT/pt_scaling/auto_scaling_output"
...
proc auto_scaling::build_config {} {
    foreach name {
        ACTION TARGET_PROCESS TARGET_VOLTAGE TARGET_TEMPERATURE SCALING_AXIS
    ...
    if {$action ni {check run}} { error "ACTION은 check 또는 run이어야 합니다." }
    ...
    return [dict create action $action config $cfg]
}

# source하면 위 설정으로 실행. load_only는 개발/검증용이며 일반 사용에는 불필요.
if {![info exists ::auto_scaling_load_only] || !$::auto_scaling_load_only} {
    set _built [auto_scaling::build_config]
    set scaling_config [dict get $_built config]
    switch -- [dict get $_built action] {
        check {
            puts "ACTION=check: 라이브러리를 PT에 로드하거나 STA를 실행하지 않고 선택만 확인합니다."
            auto_scaling::plan $scaling_config
            puts "확인이 끝났습니다. USER SETTINGS의 ACTION을 run으로 바꾸면 실행됩니다."
        }
        run { auto_scaling::run $scaling_config }
    }
}
