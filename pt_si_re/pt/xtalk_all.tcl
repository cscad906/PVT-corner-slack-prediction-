...
if {![file exists $CTX]} {
    ...
    puts "  넷 목록이 없어 리포트에서 직접 만듭니다 : $RPT"
    ...
    close $f
    puts "  만든 넷 목록   : [llength $rows] 개  ->  $CTX"
}

puts "--------------------------------------------------------------------"
puts "  이 폴더   : [pwd]"
puts "  분석      : [expr {$DELAY_TYPE eq {min} ? {hold (-min)} : {setup (-max)}}]"
puts "  SI 해석   : [expr {[get_app_var si_enable_analysis] ? {켜짐} : {꺼짐  <- crosstalk 이 전부 0 이 됩니다}}]"
puts "--------------------------------------------------------------------"
puts "  목록 읽는 중 : $CTX"
...
puts ""
puts "  물어본 넷      : $total  (정상 $ok / 실패 $err)"
puts "  긁어낸 aggressor: [dict size $aggr_nets] 개"
...
close $out
puts "  aggressor 넷  : $na  (driver 못 찾음 $na_bad)"
...
puts "  결과"
puts "--------------------------------------------------------------------"
puts [format "  %-24s %s" "물어본 넷" "$total  (정상 $ok / 실패 $err)"]
puts [format "  %-24s %s" "aggressor 넷" "[dict size $aggr_nets]  (driver 못 찾음 $na_bad)"]
puts [format "  %-24s %s" "victim 핀" "$nv  (못 찾음 $nv_bad)"]
puts [format "  %-24s %s" "PT 원문" "$RAW  ([file size $RAW] 바이트)"]
puts "--------------------------------------------------------------------"

set bad 0
if {$err > 0} {
    puts "  확인 필요 : 넷 $err 개가 실패했습니다."
    puts "              $RAW 에서 'status=ERROR' 를 찾아 message= 를 보세요."
    puts "              PIN_NOT_FOUND 가 많으면 이 코너의 넷리스트가 리포트와 다릅니다."
    puts "              에러 코드: W-XCALCERR"
    set bad 1
}
if {[dict size $aggr_nets] == 0} {
    puts "  문제 발생 : aggressor 를 하나도 못 찾았습니다."
    puts "              SI 가 꺼졌거나 SPEF 에 coupling 이 없습니다."
    puts "              printvar si_enable_analysis 로 확인하세요."
    puts "              read_parasitics 에 -keep_capacitive_coupling 이 있어야 합니다."
    puts "              에러 코드: E-NOAGGR"
    set bad 1
}
if {$na_bad > [expr {[dict size $aggr_nets] / 2}]} {
    puts "  확인 필요 : aggressor 의 절반 이상이 driver 를 못 찾았습니다."
    puts "              이름 규약이 다를 수 있습니다. 에러 코드: W-NODRIVER"
    set bad 1
}
if {$bad == 0} {
    puts "  정상 종료           \[ OK-XTALK \]"
