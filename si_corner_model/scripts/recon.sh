...
echo "date : $(date)"
echo "host : $(hostname)"
echo "root : $ROOT       <- root from config.yaml"
echo "repo : $(pwd)"
echo

echo "===== [1] directly under root = circuit folders (designs in the config) ====="
ls -la "$ROOT" 2>&1 | head -60
echo

DESIGNS="$(find "$ROOT" -maxdepth 1 -mindepth 1 -type d -not -name '.*' \
           -not -name 'si_corner_model' -printf '%f\n' 2>/dev/null | sort)"
echo "--- circuit candidates ---"; echo "$DESIGNS"
echo
...
for D in $DESIGNS; do
  ...
  echo "--- count per extension ---"
  find "$P" -maxdepth 3 -type f 2>/dev/null | sed 's/.*\.//' | sort | uniq -c | sort -rn | head -10
  echo "--- filename samples (match files.annotated_regex to these) ---"
  find "$P" -maxdepth 3 -type f 2>/dev/null | xargs -n1 basename 2>/dev/null | sort | head -25
  echo "--- corner token spread (actual voltage/temp/BEOL spelling; sets temps.levels) ---"
...
echo "=============================================================="
echo "===== [2] *** report body = decides whether the parser needs changing ***"
echo "=============================================================="
echo "sample : $SAMPLE"
if [[ -n "$SAMPLE" ]]; then
  ls -l "$SAMPLE"; wc -l "$SAMPLE"; echo
  echo "--- first 120 lines ---"; head -120 "$SAMPLE"; echo
  echo "--- keyword counts (is FIXED_PATH present / is this SSTA) ---"
  ...
  echo
  echo "--- *** is the path set identical across corners (the model assumes it) ---"
  SDIR="$(dirname "$SAMPLE")"
  for f in $(ls "$SDIR" | head -4); do
    printf '  %-50s Startpoint=%s\n' "$f" "$(grep -c 'Startpoint' "$SDIR/$f" 2>/dev/null)"
  done
  echo "  (differing counts = per-corner worst path -> fixed paths must be re-annotated)"
else
  echo "no report candidate found -> read the [1] output and set root again"
fi
echo

echo "===== [3] SI / crosstalk location (sets files.crosstalk_subdir) ====="
find "$ROOT" -maxdepth 4 \( -iname '*xtalk*' -o -iname '*crosstalk*' -o -iname '*noise*' \) 2>/dev/null | head -25
[[ -n "$SAMPLE" ]] && echo "--- are crosstalk columns mixed into the report ---" && \
