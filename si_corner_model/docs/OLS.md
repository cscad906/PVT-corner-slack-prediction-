...
| **레벨 축 보간** 코너(양옆 레벨이 둘 다 seen)가 나쁨 | `level_values` 좌표가 틀렸다. 가운데 레벨이 정가운데가 아니거나, 아예 순서가 다르다 | `fit_level_values: true` (기본) 가 좌표와 `level_order` 를 seen-LOO 로 같이 고른다. `[LEVELS]` 줄에 후보와 채택 결과가 찍히고, `[level axis ]` 줄에 원본 평균이 찍힌다 |
